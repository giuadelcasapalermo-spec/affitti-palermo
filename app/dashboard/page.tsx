...
export default function Dashboard() {
  ...
  // Dati giornalieri per i grafici per stanza
  function buildChartData(cameraId: number) {
    ...
    return Array.from({ length: nDays }, (_, i) => {
      ...
      return {
        giorno: parseInt(dayStr.slice(8)),
        valore,
        saturazione: booking ? 100 : 0,
        arrivo: isCheckIn ? 1 : 0,
      };
    });
  }
  ...
  return (
    <div className="space-y-6">
      ...
      {/* ── Grafici statistici per stanza — solo desktop ── */}
        ...
        <div className="grid grid-cols-2 xl:grid-cols-3 gap-4">
          {camere
            ...
            .map(camera => {
              ...
              const satPercent = data.length > 0
                ? Math.round((giorniOccupati / data.length) * 100)
                : 0;
              ...
              return (
                <div key={camera.id} className="bg-white rounded-lg shadow-sm p-4">
                  ...
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <span className="text-xs text-gray-400">{filtroCamera === 'tutte' ? '' : ''}{format(parseISO(filtroDal), 'MMM yyyy')}</span>
                  </div>
